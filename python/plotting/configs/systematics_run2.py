...
from Artus.Utility.tools import make_multiplication
...
class SystematicsFactory(dict):
	def __init__(self):
		...
		self["CMS_phi_1_sin_13TeV"] = MuPhiSinSystematic
		self["CMS_phi_1_cos_13TeV"] = MuPhiCosSystematic
...
class MuPhiSinSystematic(SystematicShiftBase):

	def get_config(self, shift=0.0):
            plot_config = super(MuPhiSinSystematic, self).get_config(shift=shift)

            sys_frac = 0.05*0.112
            for index, weight in enumerate(plot_config.get("weights", [])):
		if not "Run201" in plot_config["files"][index]:
                    if shift > 0.0:
                            modifier = ["1+sin(phi_1)*{}".format(sys_frac)]
                            plot_config["weights"][index] = make_multiplication([weight] + modifier)
                    elif shift < 0.0:
                            modifier = ["1-sin(phi_1)*{}".format(sys_frac)]
                            plot_config["weights"][index] = make_multiplication([weight] + modifier)
...
class MuPhiCosSystematic(SystematicShiftBase):

	def get_config(self, shift=0.0):
            plot_config = super(MuPhiCosSystematic, self).get_config(shift=shift)

            sys_frac = 0.05*0.115
            for index, weight in enumerate(plot_config.get("weights", [])):
		if not "Run201" in plot_config["files"][index]:
                    if shift > 0.0:
                            modifier = ["1+cos(phi_1)*{}".format(sys_frac)]
                            plot_config["weights"][index] = make_multiplication([weight] + modifier)
                    elif shift < 0.0:
                            modifier = ["1-cos(phi_1)*{}".format(sys_frac)]
                            plot_config["weights"][index] = make_multiplication([weight] + modifier)
